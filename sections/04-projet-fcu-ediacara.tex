...
Le \terme{fcu} vise à produire les cartes papier de manière plus automatisée à partir des données du \terme{shom}. Dans le processus historique, les cartographes travaillent avec la \terme{bdmr}, dépôt de référence des productions raster historiques, et y déposent les sources, les fichiers de géoréférencement et les éléments intermédiaires nécessaires aux traitements. Une carte \terme{fcu} arrive au contraire déjà sous la forme de produits préparés en amont, notamment des \terme{geotiff} géoréférencés et des fichiers PDF. Elle ne passe pas par la \terme{bdmr}, ne possède pas de fichier \texttt{.lis} --- fichier déclaratif décrivant notamment les éléments composant la carte --- et n'a pas de cartouches, c'est-à-dire de blocs de titre ou de texte ajoutés à la carte papier. Elle reste néanmoins référencée dans ShomProduit, référentiel des métadonnées et de l'état des produits cartographiques, et doit finalement rejoindre plusieurs espaces de diffusion communs aux cartes historiques.
...
C2.1 & Conception d'une architecture d'intégration autour de composants legacy, avec une première proposition d'architecture dédiée au dépôt et à la livraison des cartes \terme{fcu} --- nommée ArchFCU dans la suite du chapitre --- puis adaptation à la solution opérationnelle finalement retenue. \\
...
C3.2 & Validation itérative sur l'environnement de recette RE7, comparaison prolongée avec la production et vérification de non-régression. L'absence d'une suite de tests automatisés exhaustive limite la force de cette preuve. \\
...
La \terme{bdmr} est alimentée dans le processus historique par les cartographes, notamment au travers de GRASTER, une application interne développée en WinDev qui prépare et dépose les éléments de production. Ediacara ne se contente donc pas de copier des fichiers. À partir des données présentes en \terme{bdmr} et des métadonnées de produits issues de ShomProduit, elle réalise ou orchestre des contrôles de cohérence, produit des métadonnées, génère différents \terme{geotiff}, PDF, archives et fichiers destinés aux systèmes de diffusion, puis synchronise plusieurs espaces réseau.
...
Ajouter ShomProduit comme référentiel transversal des métadonnées des produits, utilisé par les contrôles.
}}
...
Le Fond de carte unifié vise à automatiser davantage la production cartographique directement à partir des données disponibles dans les bases du \terme{shom}. L'objectif métier est d'obtenir un processus plus simple, plus rapide et moins sensible aux erreurs liées aux manipulations intermédiaires. La chaîne de production située en amont d'Ediacara a elle aussi évolué ; dans ce dossier, je me concentre toutefois sur les conséquences propres à Ediacara et à la coexistence entre les productions historiques et \terme{fcu}.

Cette évolution change fortement l'entrée de la chaîne. Une carte historique possède dans la \terme{bdmr} les fichiers bruts, les éléments de production et les informations de géoréférencement nécessaires aux traitements. Une carte \terme{fcu} ne passe jamais par ce dépôt : les \terme{geotiff} sont déjà géoréférencés, les produits requis sont préparés en amont et les cartes ne comportent pas de cartouches. Les traitements de construction historique à partir des fichiers de géoréférencement ne doivent donc pas leur être appliqués. Les formats et le contenu exact de ces entrants ont par ailleurs été stabilisés tardivement pendant le projet, ce qui a imposé plusieurs ajustements de l'adaptation au fur et à mesure que le contrat d'entrée se précisait.

En revanche, le \terme{fcu} ne constitue pas un univers séparé jusqu'au bout. Les cartes sont inscrites dans ShomProduit, qui reste le référentiel commun des métadonnées des produits utilisé par plusieurs contrôles et générations de listes. Elles doivent ensuite rejoindre les mêmes espaces de diffusion que les cartes historiques : préparation pour l'impression à la demande, stockage des produits et de leurs métadonnées dans l'entrepôt, mise à disposition des \terme{geotiff} et PDF, puis publication jusqu'à la boutique du Shom. Le projet est ainsi un problème de \textbf{coexistence de deux chaînes d'alimentation vers des sorties communes}.
...
Dépôt dédié \terme{fcu} $\rightarrow$ \texttt{livraison\_fcu} $\nearrow$\\[0.2cm]
ShomProduit apparaît au centre comme référentiel commun des métadonnées des produits, utilisé pour distinguer les cartes et contrôler leur état.
}}
...
La reconstruction du serveur de recette a été suivie à l'aide d'une matrice technique, job par job. Pour chaque étape, je notais les interactions avec ShomProduit, la \terme{bdmr}, les livraisons, le répertoire de travail et les espaces locaux, puis je comparais l'exécutable, la source et les paramètres entre la production et l'environnement de recette RE7. Cette feuille ne constitue pas un planning projet complet, mais elle m'a servi de liste de contrôle pour éviter d'oublier une dépendance lors de la reproduction de l'environnement.

\begin{figure}[H]
\centering
\fbox{\parbox{0.9\textwidth}{\centering
Emplacement prévu : extrait anonymisé de la matrice environnement de recette / production.\\
Faire apparaître quelques lignes de jobs et les colonnes \og interactions réseau \fg, \og source production \fg, \og état recette \fg, \og source recette \fg et \og paramètres \fg.
}}
\caption{Matrice technique utilisée pour reconstruire et contrôler l'environnement de recette RE7}
...
Cette architecture n'a pas été mise en œuvre telle quelle. Au fil des échanges, le dépôt opérationnel a été construit plus simplement sur un partage réseau interne dédié au \terme{fcu}. Son alimentation a été prise en charge par un traitement FME développé par mon tuteur ; FME est un outil \terme{etl} spécialisé dans l'intégration et la transformation de données géospatiales. Je n'ai pas développé ce composant ; nous avons en revanche réfléchi ensemble au fonctionnement général du dépôt et j'ai proposé certains mécanismes, notamment le verrou permettant d'éviter qu'Ediacara lise une carte pendant sa mise à jour.
...
\midrule
Dépôt & Serveur Linux ArchFCU dédié. & Arborescence \terme{fcu} sur un partage réseau interne dédié. \\
Dépôt utilisateur & Application web PHP en glisser-déposer. & Traitement FME, \terme{etl} géospatial, mis à disposition des cartographes. \\
...
\centering
\fbox{\parbox{0.9\textwidth}{\centering
Emplacement prévu : schéma production / environnement de recette.\\[0.2cm]
Production : données réelles $\rightarrow$ traitements $\rightarrow$ livraisons réseau.\\
Recette RE7 : mêmes lectures utiles $\rightarrow$ traitements équivalents $\rightarrow$ destinations locales / recette.\\
Mettre en évidence la barrière sur toutes les écritures réseau.
}}
\caption{Principe d'isolation de l'environnement de recette RE7}
\label{fig:isolation-re7}
\end{figure}

Cette démarche a fait de RE7 un livrable à part entière. Elle a ensuite servi pendant plusieurs semaines pour comparer les résultats avec la production et sécuriser la migration. Certains essais dépendaient néanmoins de données de test préparées dans des référentiels utilisés par la production ; ils nécessitaient donc une remise en état contrôlée de ces données après l'essai. Cette contrainte est une limite de l'isolation : les écritures de livraison étaient cloisonnées, mais tous les référentiels amont n'avaient pas d'équivalent de recette autonome.
...
\begin{enumerate}
    ...
    \item \textbf{ajouter un flux de livraison \terme{fcu} autonome en fin de chaîne}, à partir du dépôt dédié aux productions \terme{fcu}.
\end{enumerate}
...
\midrule
\path{XML_Shom-produits} & Modifié / réécrit & Conserver un catalogue global tout en excluant les \terme{fcu} des XML unitaires du flux historique. \\
\path{liste_imdem_webservices} & Réécrit & Exclure les \terme{fcu} de la liste IMDEM historique. \\
\path{cp_imp} & Réécrit en Python & Réconcilier IMP/IMDEM sans supprimer ni déplacer les produits \terme{fcu}. \\
\path{cp_gt} et \path{cp_gtw} & Réécrits en Python & Synchroniser les \terme{geotiff} natifs et WGS84 en préservant les \terme{fcu}. \\
\path{copy_geotiffs_direct} & Configuration modifiée & Conserver l'exécutable Eiffel mais rediriger sa sortie vers un tampon local. \\
\path{make_pdfs} & Configuration modifiée & Générer les PDF dans un tampon local plutôt que directement dans la livraison. \\
\path{livre_geotiffs_direct_et_pdf_direct} & Nouveau job Python & Effectuer la synchronisation contrôlée des deux tampons vers le réseau. \\
\path{copy_entrepot_mtd/data} & Réécrits / modernisés & Fiabiliser l'alimentation de l'entrepôt depuis les produits de la chaîne. \\
\path{livraison_fcu} & Nouveau job Python & Contrôler le dépôt \terme{fcu} puis livrer chaque carte vers les destinations communes. \\
\bottomrule
\end{tabular}
...
Le nouveau job \texttt{livraison\_fcu} est exécuté à la fin de la chaîne NP. Sa source n'est pas la \terme{bdmr} mais le dépôt réseau dédié aux productions \terme{fcu}. L'arborescence distingue les cartes en service, les retirages et les cartes supprimées. Pour une carte active, les niveaux \texttt{numero/edition/correction} permettent d'identifier précisément la version à traiter.
...
La copie est elle aussi conditionnelle. Lorsqu'une empreinte MD5 de destination correspond déjà à celle du dépôt, aucune écriture n'est réalisée. Lorsqu'un fichier est absent ou différent, il est recopié. Le job alimente ainsi les espaces de \terme{geotiff} destinés aux consommateurs, les PDF, l'impression à la demande, les versions native et WGS84 ainsi que les répertoires de l'entrepôt de données et de métadonnées. Les cartes placées dans l'espace \texttt{supprimees} du dépôt \terme{fcu} font l'objet d'une purge ciblée de leurs artefacts connus.
...
Dépôt \terme{fcu} $\rightarrow$ contrôle version $\rightarrow$ lock $\rightarrow$ présence $\rightarrow$ MD5 $\rightarrow$ XML\\
$\rightarrow$ copie conditionnelle / génération PDF $\rightarrow$ impression, PDF, \terme{geotiff}, entrepôt et publication.\\
...
Dans l'état actuel de la chaîne, un traitement dédié à Cartes-en-ligne complète également le flux. À partir du contenu de l'entrepôt \terme{fcu}, il génère le CSV attendu par l'application, prépare une archive ZIP avec les informations géographiques nécessaires, compare les dates et tailles avec l'état publié et n'envoie que les mises à jour utiles. Cette séparation permet de conserver les mécanismes de publication existants jusqu'à la mise à disposition sur la boutique du Shom, sans réintroduire les \terme{fcu} dans la chaîne \terme{bdmr}.

\section{Validation et non-régression}

\subsection{Une recette prolongée plutôt qu'une campagne automatisée}

La validation de \terme{fcu} ne s'est pas appuyée sur une suite automatisée exhaustive. Ce point constitue une limite du projet et je ne souhaite pas le présenter autrement. La stratégie réellement utilisée a consisté à faire fonctionner l'environnement de recette RE7 pendant plusieurs semaines avec une configuration aussi proche que possible de la production, mais avec les écritures de livraison isolées.
...
\midrule
Exécution de la chaîne historique sur l'environnement de recette RE7 & Les livrables, métadonnées et actions restent équivalents à la production. & Validé sur plusieurs semaines \\
Isolation des livraisons de recette & Aucune écriture de test n'atteint les espaces de diffusion de production. Certains référentiels amont restent toutefois partagés et nécessitent une remise en état après les essais concernés. & Validé avec cette limite \\
...
Contrôle du dépôt \terme{fcu} & Les fichiers manquants, MD5 incohérents, XML incohérents ou cartes verrouillées empêchent la livraison concernée. & Fonctionnel \\
...
Cette approche était adaptée au risque immédiat de non-régression, mais elle a un coût : elle demande du temps d'observation et dépend davantage de la connaissance de l'opérateur. Si je reprenais ce projet, je compléterais plus tôt l'environnement de recette par des tests automatisés sur les fonctions Python critiques, notamment la détection \terme{fcu}, les synchronisations, les contrôles d'archive et les règles de suppression.
...
La production a également été migrée vers le nouvel environnement Python 3.12 et les sources locales versionnées dans Git. Après les validations sur l'environnement de recette RE7, l'intervention a finalement été réalisée à la fin de l'année 2025, quelques semaines après la date initialement envisagée. Les six cartes \terme{fcu} actuellement en service ont depuis été testées et livrées par cette architecture.
...
La mise en production n'a pas marqué la fin du projet. Je consulte quotidiennement l'état des jobs Jenkins afin de repérer rapidement les erreurs et j'interviens lorsque les retours d'un consommateur révèlent une différence de comportement. La supervision repose donc encore largement sur la lecture de l'état des jobs, de leurs logs et sur les remontées opérationnelles des consommateurs, plutôt que sur un dispositif d'alerting centralisé décrit comme un livrable du projet. Cette phase est particulièrement importante sur une chaîne historique, car certaines règles ne sont pas formalisées et n'apparaissent qu'en observant les produits réellement utilisés.
...
En inspectant le fichier généré puis le comportement de l'ancien code Eiffel, j'ai trouvé une règle qui n'avait pas été reproduite : le PDF \terme{gdal} contenait une entrée \texttt{/UserUnit 4.1666661}. Dans un PDF, \texttt{/UserUnit} multiplie l'unité graphique standard de 1/72 de pouce. La valeur générée ici est proche du rapport $300/72$ ; si elle est interprétée comme un facteur d'échelle physique par le lecteur PDF, la page apparaît environ 4,17 fois trop grande. Le traitement historique neutralisait cet effet en remplaçant \texttt{/UserUnit} par \texttt{1}, ce qui restaure l'unité PDF standard sans modifier le contenu raster déjà produit à la résolution attendue.
...
Le travail de rétro-ingénierie m'a ensuite permis d'intervenir sur des problèmes qui ne concernaient pas directement \terme{fcu}. Un exemple est la désynchronisation possible entre les titres de cartouches présents dans un fichier \texttt{.lis} et ceux déclarés dans ShomProduit. Le fichier \texttt{.lis} est un fichier déclaratif historique associé à la carte : il recense les éléments à produire, notamment les cartouches, c'est-à-dire les blocs de titre ou de texte qui complètent la carte papier. La chaîne produit certaines métadonnées en combinant cette description avec les données de ShomProduit et tente d'associer les cartouches à partir des titres, notamment à l'aide d'une distance de Levenshtein. Un écart de saisie entre ces deux sources peut donc provoquer un dysfonctionnement alors que le code lui-même n'est pas la cause initiale.

Ce type d'incident m'a appris à ne pas limiter le diagnostic au job en erreur. Sur Ediacara, il faut souvent remonter jusqu'à la provenance métier d'une donnée, comparer les sources qui la décrivent puis déterminer si la correction relève du logiciel ou de l'alignement des données. Le fait qu'une même information puisse être portée à la fois par le fichier \texttt{.lis} et par ShomProduit constitue d'ailleurs une limite d'urbanisation : idéalement, un référentiel maître unique devrait porter cette information et les autres composants ne devraient qu'en dériver leur représentation.
...
\begin{itemize}
    ...
    \item la matrice environnement de recette / production, utilisée pendant la reconstruction et la migration ;
...
\begin{itemize}
    \item un environnement de recette RE7 reconstruit et isolé pour tester les évolutions ;
...
La validation gagnerait par ailleurs à être davantage automatisée. L'environnement de recette et les comparaisons prolongées ont permis une mise en production maîtrisée, mais une vraie suite de tests unitaires et d'intégration rendrait les réécritures futures moins coûteuses. La surveillance après déploiement repose encore en partie sur la consultation des jobs et l'analyse manuelle des logs.
