...
Le bloc~1 repose principalement sur FCU / Ediacara et Lynx Immo. Ediacara démontre l'analyse d'un environnement professionnel complexe, l'identification des risques et la conception d'une adaptation compatible avec la production existante. Lynx Immo complète cette dimension par une étude de faisabilité intégrant les aspects techniques, humains et financiers, puis par une feuille de route, des jalons, des outils de suivi et des arbitrages de périmètre. La veille technologique y est désormais présentée comme un système de décision : périmètre défini, sources officielles anglophones, critères de comparaison, déclencheurs de réévaluation et compte rendu en annexe. Elle montre notamment comment les hypothèses initiales en faveur de React Native et Django ont été rejouées lorsque le besoin réel a conduit à retenir Flutter, React, Supabase/PostgreSQL et RevenueCat. Le homelab conserve un rôle complémentaire d'expérimentation sur les pratiques DevOps et d'observabilité.

\subsection*{Bloc 2 --- Concevoir et développer des solutions logicielles}

Les preuves sont volontairement réparties entre plusieurs projets. S-123 illustre principalement la conception d'architecture et l'évolution d'un modèle devenu trop couplé à la norme. \texttt{piv2glz} apporte une preuve claire d'intégrité du code par les tests automatisés et la CI. Brise-glace constitue la preuve principale du front-end avec React, TypeScript et une interface temps réel, tandis que FeastVerse démontre le développement back-end avec Spring Boot, PostgreSQL, JWT et une architecture en couches. Le projet de prédiction des coûts d'assurance porte C2.5 en démontrant une chaîne complète de valorisation de la donnée, depuis l'exploration et la préparation jusqu'à l'entraînement, l'évaluation, l'explicabilité, l'exposition du modèle et la persistance des résultats. La dimension de traitement à grande échelle y est abordée comme une conception d'évolution de l'architecture, en distinguant clairement la solution effectivement réalisée de celle que j'adopterais lorsque la volumétrie et la fréquence des flux rendraient les traitements locaux insuffisants.
...
C1.2 & Veille technologique & Lynx Immo & p.~\pageref{sec:lynx-veille} \\
...
C2.5 & Traitement de données & Prédiction des coûts d'assurance & p.~\pageref{chap:data-ia} \\
\midrule
...
La couverture du référentiel repose ainsi sur des projets complémentaires, avec des niveaux de preuve différents selon les contextes. Les perspectives identifiées ne remettent pas en cause les réalisations présentées : elles servent à expliciter les limites rencontrées et la manière dont les pratiques pourraient être renforcées dans un cadre d'exploitation plus large, notamment pour l'accessibilité, le numérique responsable ou la formalisation de certains dispositifs de suivi. La veille Lynx Immo constitue désormais une preuve directe de C1.2 et de la compétence transversale d'anglais technique grâce au compte rendu en français fondé sur des documentations officielles anglophones.
