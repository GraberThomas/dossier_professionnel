...
La présentation qui suit croise le rapport collectif remis à la fin du projet avec le code effectivement conservé sur la branche \texttt{main} du dépôt GitHub \textit{CollectiveEscape}. La version technique relue correspond au commit \texttt{57a33a2}. Cette double source permet de distinguer les intentions décrites dans le rapport des fonctions réellement présentes dans l'application.
...
Le client a été réalisé avec React 18, TypeScript 5.6 et Vite 5. React répondait au besoin de construire une interface composée de nombreux états et composants réutilisables. TypeScript apportait un typage statique utile pour sécuriser les échanges avec les données générées à partir du schéma PostgreSQL. Vite offrait un environnement de développement rapide et une production optimisée du client.

Le back-end repose sur Supabase, qui réunit une base PostgreSQL, un système d'authentification, un stockage de fichiers, des API générées à partir du schéma et des canaux temps réel. Les illustrations des scénarios sont notamment récupérées depuis un compartiment de stockage Supabase. Ce choix permettait d'éviter le développement d'un serveur applicatif séparé dans un projet limité à trois mois. Il restait néanmoins nécessaire de concevoir le modèle, d'écrire les migrations, de définir les règles de sécurité et de développer les fonctions SQL portant les opérations métier complexes.
...
Navigateur React/TypeScript -- Contextes et services -- Supabase Auth, Storage, Realtime et PostgreSQL.\\
Vercel construit et publie séparément le client web.
}}
...
J'ai défini progressivement des politiques RLS pour encadrer les opérations de lecture et d'écriture. Leur objectif était qu'un utilisateur ne puisse pas modifier le scénario privé d'un autre créateur, agir à la place du maître du jeu ou consulter une archive à laquelle il n'avait pas participé. Le contrôle est exécuté par PostgreSQL, indépendamment de ce que l'interface affiche. Cette défense au niveau des données limite le risque qu'un appel fabriqué manuellement contourne les restrictions du client.

La relecture du dépôt montre toutefois que cette sécurisation n'était pas complètement aboutie. Dans la migration initiale, les politiques de modification et de suppression des éléments d'un scénario vérifient que l'utilisateur possède un scénario, mais ne relient pas toujours ce contrôle au scénario de l'élément concerné. Une politique correctement corrélée aurait dû comparer l'identifiant de ce scénario à \texttt{ScenarioElement.scenarioID}. Ce défaut n'a pas été détecté pendant le projet, faute de tests d'intégration dédiés aux politiques RLS. Il constitue une limite importante et montre qu'une intention de sécurité ne suffit pas sans scénarios de contrôle positifs et négatifs.

L'archivage illustre également la nécessité de déplacer certaines règles au plus près de la base. La fonction PostgreSQL \texttt{archive\_game} vérifie que l'appelant est le maître du jeu et que la partie est terminée. Elle crée ensuite l'archive, enregistre les associations avec les joueurs, puis supprime la partie des tables actives. Le traitement regroupe ainsi une opération métier cohérente au lieu de laisser le client enchaîner plusieurs écritures susceptibles d'être interrompues. En revanche, l'interface de consultation n'a pas été terminée et les dernières politiques du dépôt n'exploitent pas encore complètement l'association \texttt{UserCanReadArchive} pour ouvrir l'archive aux participants. Le mécanisme de production de l'archive est donc présent, mais le parcours de consultation reste incomplet.
...
\begin{itemize}
    ...
    \item un hook \texttt{useTheme} et le \texttt{ThemeProvider} fourni par React Bootstrap pour basculer l'attribut \texttt{data-bs-theme} entre les modes clair et sombre ;
...
Les deux contextes applicatifs réellement développés, \texttt{AuthContext} et \texttt{GameContext}, évitent que chaque écran réimplémente la connexion à Supabase ou conserve une copie indépendante de l'état global. Le thème ne constitue pas un troisième contexte métier : il s'appuie sur le mécanisme de thème de Bootstrap et sur un hook spécialisé. Cette séparation facilite l'évolution du client : un composant d'interface consomme une donnée ou déclenche une action sans avoir à connaître tous les détails de persistance.

\subsection{Parcours utilisateur et cohérence visuelle}

La page d'accueil regroupe les trois entrées principales du produit : rejoindre une partie avec un code, créer une partie et consulter les scénarios disponibles. Lors du chargement, l'application recherche également une éventuelle partie en cours afin de permettre à un joueur déconnecté de reprendre plus facilement sa session. La consultation des scénarios publics et privés est fonctionnelle, mais le bouton de création d'un scénario ne déclenche encore aucune action. Les méthodes de création, de modification et de suppression existent dans le service TypeScript, sans être reliées à un parcours d'interface complet.
...
Le classement des quinze objets constitue l'interaction centrale du jeu. Une première expérimentation isolée a permis à l'équipe d'étudier le comportement attendu : déplacer une carte, la déposer à une position, modifier l'ordre, retirer une carte et réinitialiser le classement. J'ai ensuite repris ces apprentissages pour intégrer la fonctionnalité dans l'application principale React et TypeScript avec la bibliothèque \texttt{react-dnd}.

L'implémentation conserve séparément les identifiants placés dans les quinze emplacements et les détails immuables des cartes. Sur ordinateur, \texttt{HTML5Backend} gère le glisser-déposer. Sur un écran tactile, le client détecte le périphérique et utilise \texttt{TouchBackend}, tout en proposant surtout une interaction plus fiable consistant à sélectionner une carte puis à toucher son emplacement de destination. Le passage de quelques cartes de test aux quinze objets du scénario a également imposé de travailler la lisibilité de l'écran et la séparation des styles. Les cartes associent une illustration et un intitulé afin de réduire l'effort d'identification pendant la discussion.

Cette adaptation tactile est un résultat concret, confirmé par les derniers commits de compatibilité mobile. Elle ne résout cependant pas l'accessibilité au sens large. L'interface a été pensée pour être compréhensible et responsive, mais elle n'a pas fait l'objet d'un audit WCAG formalisé. Une version plus aboutie devrait proposer une alternative utilisable entièrement au clavier et avec un lecteur d'écran, vérifier systématiquement les contrastes et tester le parcours avec différents profils d'utilisateurs. Ce constat limite la portée de la preuve en matière d'inclusion, mais il constitue également un enseignement concret pour mes futurs développements front-end.

\section{Synchronisation et déroulement d'une partie}

\subsection{Modélisation des états}

Le cœur de l'application repose sur une machine d'états simple. Le parcours effectivement géré par l'interface utilise les états \texttt{WAITING}, \texttt{ACTIVE} et \texttt{FINISHED}. Lorsqu'une partie est active, une étape supplémentaire distingue la réflexion individuelle, le travail en petit groupe et la mise en commun. Cette représentation rend explicites les transitions autorisées et permet à l'interface de choisir le composant adapté. L'énumération TypeScript contient également un état \texttt{CLOSED}, mais celui-ci n'est pas traité dans le composant principal : il correspond à une évolution envisagée plutôt qu'à un parcours finalisé.
...
Des URL de prévisualisation sont également créées pour les branches et les demandes de fusion. Elles permettent de vérifier une évolution dans un environnement distant sans remplacer la version principale. Les journaux de construction facilitent enfin le diagnostic lorsqu'une installation ou une compilation échoue. Le dépôt contient un fichier \texttt{vercel.json} chargé de rediriger les routes de l'application monopage vers \texttt{index.html}. Le lien entre GitHub et Vercel reste toutefois une configuration externe : aucun workflow GitHub Actions n'est présent dans le dépôt.
...
Le dépôt configure Vitest et expose une commande \texttt{npm run test}. Il contient cependant un seul fichier de test, nommé \texttt{ScenarioService.test.ts}, dont l'unique assertion vérifie simplement que la valeur \texttt{true} est vraie. Le commentaire du fichier le présente lui-même comme un test factice destiné à laisser passer la chaîne. Il ne valide donc pas le service annoncé par son nom et ne constitue pas une couverture automatisée significative.

Le rapport collectif évoque plus largement des tests unitaires, d'intégration et de charge automatisés, mais le code conservé ne permet pas d'étayer cette affirmation. La validation s'est surtout appuyée sur le \textit{build} TypeScript, l'analyse statique ESLint, les essais fonctionnels au fil des développements et les scénarios exécutés avec plusieurs clients connectés. Le script de production exécute \texttt{tsc -b} puis le \textit{build} Vite, ce qui détecte les erreurs de typage et de compilation sans vérifier le comportement métier.
...
Cette stratégie a permis de livrer une version démontrable, mais elle demeure insuffisamment industrialisée. Un nouveau cycle devrait remplacer le test factice par des tests unitaires sur les calculs et les transitions d'état, ajouter des tests d'intégration sur les politiques RLS et les fonctions SQL, puis des tests de bout en bout simulant plusieurs navigateurs.
...
Ce projet constitue ainsi ma preuve principale de C2.3. Il montre ma capacité à réaliser une interface React dynamique, à intégrer des interactions complexes et à la relier à un système temps réel dont la sécurisation a été engagée par des politiques RLS. La relecture du dépôt conduit toutefois à ne pas qualifier cette sécurisation de complète. Elle précise mes prochains axes de progression : accessibilité, tests automatisés substantiels, vérification systématique des autorisations et partage de la réalisation.
